    ...
    <div className="mt-5 space-y-5">
      <div className="panel animate-pop relative overflow-hidden px-6 py-10 text-center">
        <div
          ...
          style={{ background: 'radial-gradient(circle, rgba(182,255,59,0.28), transparent 65%)' }}
        />
        <div className="relative">
          ...
          {champion && (
            <p className="mt-3 text-lg text-white/70">
              <span className="text-3xl font-black text-lime-300">{totalPoints(champion)}</span>{' '}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {ranked.map((p, index) => {
          ...
          return (
            <section
              key={p.id}
              className={`panel animate-rise p-5 ${index === 0 ? 'border-lime-300/40' : ''}`}
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="shrink-0 text-right">
                  <p className="text-2xl font-black leading-none text-lime-300">{points}</p>
              ...
              <div className="mt-1 flex flex-wrap gap-x-3 text-sm text-white/50">
                ...
                {complete && <span className="text-lime-300">· Equipo completo ✓</span>}
              </div>
              ...
              <ul className="mt-4 space-y-2">
                {p.team_players.map((s) => (
                  ...
                  >
                    ...
                    <div className="shrink-0 text-right">
                      <p className="font-black text-lime-300">{s.rating ?? '—'}</p>
