-- ============================================================
-- 0038: Build the whole room state in one round trip
--
-- The state endpoint is what every client asks for every five seconds, and it
-- was issuing up to nine sequential queries: settle, room, round, me, my
-- effects, all effects, the player, the envelope, the tip. Each is a separate
-- trip to the database — measured at 581 ms median, against a 1.7 KB payload.
-- The cost was never bandwidth, it was the round trips.
--
-- Assembling it here also puts the secrecy rules next to the data they
-- protect: which silhouette a given viewer may see is decided in the same
-- query that reads it.
-- Safe to re-run.
-- ============================================================
